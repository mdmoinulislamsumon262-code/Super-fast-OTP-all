...
def _temp_mail_sms_text(subject, otp) -> str:
    """Render only the OTP essentials in a welcoming SMS-style card."""
    line = "━" * 22
    has_otp = bool(otp) and str(otp).lower() != "not found"
    parts = [
        f"💌 <b>{stylish('WELCOME')}</b> 💌",
        "আপনার নতুন OTP এসে গেছে!",
        line,
    ...
    parts += [
        line,
        f"🔷 {stylish('DEVELOPER BY')} <b>{stylish(_Developer_By())}</b>",
    ]
    return "\n".join(parts)
...
async def _check_temp_mail_for_user(user_id: int):
    ...
    async with user_lock:
        ...
        for message in messages:
            ...
            notification = _temp_mail_sms_text(subject, otp)
