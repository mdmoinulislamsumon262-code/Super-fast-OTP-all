...
def init_shop_db():
    with conn() as c:
        ...
        CREATE TABLE IF NOT EXISTS shop_ms_accounts (
            user_id INTEGER PRIMARY KEY,
            email TEXT DEFAULT '',
        ...
        CREATE TABLE IF NOT EXISTS shop_oauth_states (
...
MENUS = {
    "main": lambda admin: [
        [L("🛍️", "Products"), L("🔐", "Get Code")],
        [L("📋", "My Orders"), L("💰", "Shop Balance")],
        [L("🎁", "Offers & Bonus"), L("🆘", "Shop Support")],
    ...
    "getcode": lambda admin: [
        [L("📧", "Mail Code"), L("🔑", "2FA Code")],
        [BACK()],
    ],
...
MENU_TITLES = {
    ...
    "getcode": lambda: f"🔐 <b>{st('GET CODE')}</b>\n{sep()}\n{st('Choose a code type below.')}",
...
def _handle_user_menu_text(message, menu, text) -> bool:
    ...
    if menu == "main":
        if text == L("🛍️", "Products"):
            ...
            render_menu(chat_id, uid, "getcode")
    ...
    if menu == "getcode":
        if text == L("📧", "Mail Code"):
            send_mail_code_panel(chat_id, uid)
        elif text == L("🔑", "2FA Code"):
            start_2fa_flow(chat_id, uid)
        else:
            return False
        return True
...
def _handle_user_state(message, state) -> bool:
    ...
    if step == "user_2fa_key":
        shop_states.pop(key, None)
        handle_2fa_key_input(chat_id, uid, text, message)
        return True
...
# ═══════════════════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════════════════
# GET CODE  —  Mail Code (Microsoft OAuth) + 2FA Code (TOTP)
# ═══════════════════════════════════════════════════════════════════════════
...
MS_SCOPES = "offline_access openid profile email Mail.Read User.Read"
OAUTH_STATE_TTL = 900          # 15 minutes
TOTP_CACHE_TTL = 300           # 5 minutes, memory only

_totp_cache: dict = {}         # user_id -> (secret_bytes, digits, period, expires_at)


class MailAuthError(Exception):
    """Raised when the mailbox authorization is missing, expired or revoked."""


def _envv(name, default=""):
    return (_os.getenv(name) or default).strip()


def ms_client_id():
    return _envv("MS_CLIENT_ID")


def ms_client_secret():
    return _envv("MS_CLIENT_SECRET")


def ms_tenant():
    return _envv("MS_TENANT", "common") or "common"


def _public_base_url():
    base = _envv("PUBLIC_BASE_URL") or _envv("RENDER_EXTERNAL_URL")
    return base.rstrip("/")


def ms_redirect_uri():
    explicit = _envv("MS_REDIRECT_URI")
    if explicit:
        return explicit
    base = _public_base_url()
    return (base + "/oauth/microsoft/callback") if base else ""


def ms_configured() -> bool:
    return bool(ms_client_id() and ms_client_secret() and ms_redirect_uri().startswith("https://"))


def _ms_token_url():
    return "https://login.microsoftonline.com/%s/oauth2/v2.0/token" % ms_tenant()


def _ms_authorize_url(state):
    params = {
    ...
    return "https://login.microsoftonline.com/%s/oauth2/v2.0/authorize?%s" % (
        ms_tenant(), _urlparse.urlencode(params))


# ── token at-rest protection (never plain text in the database) ────────────
def _seal_key() -> bytes:
    base = (_os.getenv("BOT_TOKEN") or "") + "|shop-mail-oauth"
    return _hashlib.sha256(base.encode("utf-8")).digest()


def _keystream(nonce: bytes, length: int) -> bytes:
    out, counter, key = b"", 0, _seal_key()
    while len(out) < length:
        out += _hmac.new(key, nonce + _struct.pack(">I", counter), _hashlib.sha256).digest()
        counter += 1
    return out[:length]


def seal_token(value: str) -> str:
    if not value:
        return ""
    raw = value.encode("utf-8")
    nonce = _secrets.token_bytes(12)
    blob = bytes(a ^ b for a, b in zip(raw, _keystream(nonce, len(raw))))
    return _b64.b64encode(nonce + blob).decode("ascii")


def unseal_token(value: str) -> str:
    if not value:
        return ""
    try:
        data = _b64.b64decode(value.encode("ascii"))
        nonce, blob = data[:12], data[12:]
        raw = bytes(a ^ b for a, b in zip(blob, _keystream(nonce, len(blob))))
...
# ── account storage ────────────────────────────────────────────────────────
def ms_get_account(user_id):
    with conn() as c:
        row = c.execute("SELECT * FROM shop_ms_accounts WHERE user_id=?", (user_id,)).fetchone()
    return dict(row) if row else None


def ms_save_account(user_id, email, refresh_token, access_token, expires_at):
    with conn() as c:
        c.execute(
            "INSERT INTO shop_ms_accounts (user_id, email, refresh_token, access_token, expires_at, updated_at) "
            "VALUES (?,?,?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET "
            "email=excluded.email, refresh_token=excluded.refresh_token, "
            "access_token=excluded.access_token, expires_at=excluded.expires_at, "
            "updated_at=excluded.updated_at",
            (user_id, email or "", seal_token(refresh_token), seal_token(access_token),
             int(expires_at), now_ts()),
        )


def ms_delete_account(user_id):
    with conn() as c:
        c.execute("DELETE FROM shop_ms_accounts WHERE user_id=?", (user_id,))


def _new_oauth_state(user_id, chat_id) -> str:
    state = _secrets.token_urlsafe(24)
    with conn() as c:
        c.execute("DELETE FROM shop_oauth_states WHERE created_at < ?", (now_ts() - OAUTH_STATE_TTL,))
        c.execute("INSERT INTO shop_oauth_states (state, user_id, chat_id, created_at) VALUES (?,?,?,?)",
                  (state, int(user_id), int(chat_id), now_ts()))
    return state


def _take_oauth_state(state):
    with conn() as c:
        row = c.execute("SELECT * FROM shop_oauth_states WHERE state=?", (state,)).fetchone()
        if row:
            c.execute("DELETE FROM shop_oauth_states WHERE state=?", (state,))
    if not row:
        return None
    row = dict(row)
    if now_ts() - int(row["created_at"]) > OAUTH_STATE_TTL:
        return None
    return row
...
def _token_request(payload):
    ...
    payload["redirect_uri"] = ms_redirect_uri()
    payload["scope"] = MS_SCOPES
    resp = _requests.post(_ms_token_url(), data=payload, timeout=25)
    try:
        body = resp.json()
    except Exception:
        body = {"error": "invalid_response", "error_description": resp.text[:200]}
    if resp.status_code >= 400 or "access_token" not in body:
        raise MailAuthError(str(body.get("error_description") or body.get("error") or "authorization failed"))
    return body
...
def _graph_get(access_token, path, params=None):
    ...
    if resp.status_code in (401, 403):
        raise MailAuthError("mailbox access is not authorized (%s)" % resp.status_code)
    if resp.status_code >= 400:
        raise MailAuthError("Microsoft API error %s" % resp.status_code)
    return resp.json()


def ms_access_token(user_id) -> str:
    """Return a valid access token, refreshing when needed. Never leaves the server."""
    if not ms_configured():
        raise MailAuthError("Microsoft sign-in is not configured on this bot yet.")
    acc = ms_get_account(user_id)
    if not acc:
        raise MailAuthError("No mailbox connected.")
    token = unseal_token(acc.get("access_token") or "")
    if token and int(acc.get("expires_at") or 0) - 90 > now_ts():
        return token
    refresh = unseal_token(acc.get("refresh_token") or "")
    if not refresh:
        ms_delete_account(user_id)
        raise MailAuthError("Authorization expired. Please sign in again.")
    try:
        body = _token_request({"grant_type": "refresh_token", "refresh_token": refresh})
    except MailAuthError:
        ms_delete_account(user_id)
        raise MailAuthError("Authorization expired or revoked. Please sign in again.")
    new_refresh = body.get("refresh_token") or refresh
    expires_at = now_ts() + int(body.get("expires_in") or 3600)
    ms_save_account(user_id, acc.get("email") or "", new_refresh, body["access_token"], expires_at)
    return body["access_token"]


def handle_oauth_callback(path):
    """Called by the web server for /oauth/microsoft/callback. Returns (status, html)."""
    query = _urlparse.parse_qs(_urlparse.urlparse(path).query)

    def page(title, detail):
        return (
            "<!doctype html><html><head><meta charset='utf-8'>"
            "<meta name='viewport' content='width=device-width,initial-scale=1'>"
            "<title>%s</title></head>"
            "<body style='font-family:system-ui,sans-serif;background:#0f1115;color:#f2f4f8;"
            "display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0'>"
            "<div style='max-width:420px;padding:28px;text-align:center'>"
            "<h2 style='margin:0 0 12px'>%s</h2><p style='opacity:.8;line-height:1.5'>%s</p>"
            "</div></body></html>" % (_html.escape(title), _html.escape(title), _html.escape(detail))
        )

    err = (query.get("error") or [""])[0]
    state = (query.get("state") or [""])[0]
    code = (query.get("code") or [""])[0]
    row = _take_oauth_state(state) if state else None
    if not row:
        return 400, page("Link expired", "This sign-in link is invalid or expired. Start again from the bot.")
    if err or not code:
        try:
            send(row["chat_id"], "⛔ %s" % st("Microsoft sign-in was cancelled or denied."))
        except Exception:
            pass
        return 400, page("Sign-in cancelled", "You can start again from the bot at any time.")
    try:
        body = _token_request({"grant_type": "authorization_code", "code": code})
        access = body["access_token"]
        profile = _graph_get(access, "/me", {"$select": "mail,userPrincipalName,displayName"})
        email = profile.get("mail") or profile.get("userPrincipalName") or ""
        ms_save_account(row["user_id"], email, body.get("refresh_token") or "",
                        access, now_ts() + int(body.get("expires_in") or 3600))
    except MailAuthError as exc:
        logger.warning("Microsoft OAuth exchange failed: %s", exc)
        try:
            send(row["chat_id"], "⛔ %s\n<i>%s</i>" % (st("Microsoft sign-in failed."), esc(str(exc))))
        except Exception:
            pass
        return 400, page("Sign-in failed", "Please try again from the bot.")
    try:
        send(row["chat_id"],
             "✅ <b>%s</b>\n%s\n<blockquote>📧 %s: <code>%s</code></blockquote>\n%s" % (
                 st("MAILBOX CONNECTED"), sep(), st("Mailbox"), esc(email),
                 st("Open Get Code → Mail Code and tap Check New Code.")))
    except Exception:
        pass
    return 200, page("Mailbox connected", "You can close this page and go back to Telegram.")


# ── verification code extraction ───────────────────────────────────────────
_CODE_PATTERNS = [
    _re.compile(r"(?:code|otp|pin|password|passcode|verification)\D{0,20}?\b([0-9]{4,8})\b", _re.I),
    _re.compile(r"(?:code|otp)\D{0,20}?\b([A-Z0-9]{5,8})\b"),
...
def _clean_preview(text, limit=220):
    body = _re.sub(r"<[^>]+>", " ", str(text or ""))
    body = _re.sub(r"\s+", " ", _html.unescape(body)).strip()
    return body[:limit]


def ms_fetch_messages(user_id, top=8):
    token = ms_access_token(user_id)
    data = _graph_get(token, "/me/messages", {
        "$top": int(top),
...
# ── Mail Code UI ───────────────────────────────────────────────────────────
def _mail_panel_keyboard(connected):
    kb = InlineKeyboardMarkup(row_width=1)
    if connected:
        kb.add(InlineKeyboardButton("🔄 %s" % st("Check New Code"), callback_data="shopcode:mail:check"))
        kb.add(InlineKeyboardButton("🚪 %s" % st("Disconnect Mailbox"), callback_data="shopcode:mail:logout"))
    else:
        kb.add(InlineKeyboardButton("🔐 %s" % st("Sign in with Microsoft"), callback_data="shopcode:mail:login"))
    return kb


def send_mail_code_panel(chat_id, user_id):
    if not ms_configured():
        send(chat_id,
             "⚠️ <b>%s</b>\n%s\n%s" % (st("MAIL CODE UNAVAILABLE"), sep(),
                                       st("Microsoft sign-in is not configured yet. Please contact the admin.")))
        return
    acc = ms_get_account(user_id)
    if acc:
        body = ("📧 <b>%s</b>\n%s\n<blockquote>✅ %s: <code>%s</code></blockquote>\n%s" % (
            st("MAIL CODE"), sep(), st("Connected mailbox"), esc(acc.get("email") or "-"),
            st("Tap Check New Code to read the latest verification message.")))
    else:
        body = ("📧 <b>%s</b>\n%s\n%s\n\n<i>%s</i>" % (
            st("MAIL CODE"), sep(),
            st("Sign in with your own Microsoft account to read verification codes from your inbox."),
            st("No password or token is ever sent in this chat.")))
    send(chat_id, body, reply_markup=_mail_panel_keyboard(bool(acc)))


def send_mail_login_link(chat_id, user_id):
    state = _new_oauth_state(user_id, chat_id)
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("🔐 %s" % st("Sign in with Microsoft"), url=_ms_authorize_url(state)))
    send(chat_id,
         "🔐 <b>%s</b>\n%s\n%s\n\n<i>%s</i>" % (
             st("MICROSOFT SIGN IN"), sep(),
             st("Tap the button below and approve mailbox read access."),
             st("The link is valid for 15 minutes and works only for you.")),
         reply_markup=kb)


def send_latest_mail_code(chat_id, user_id):
    try:
        messages = ms_fetch_messages(user_id)
    except MailAuthError as exc:
        send(chat_id, "⛔ <b>%s</b>\n%s\n%s" % (st("MAILBOX ERROR"), sep(), esc(str(exc))),
             reply_markup=_mail_panel_keyboard(False))
        return
    except Exception as exc:
        logger.warning("mail code fetch failed for %s: %s", user_id, exc)
        send(chat_id, "⛔ %s" % st("Could not read the mailbox right now. Please try again."))
        return
    if not messages:
        send(chat_id, "📭 %s" % st("No new messages found."),
             reply_markup=_mail_panel_keyboard(True))
        return
    for msg in messages:
        code = extract_verification_code(msg.get("bodyPreview") or msg.get("subject") or "")
        if not code:
            continue
        ...
        send(chat_id,
             ...
             "👤 %s: %s\n📝 %s: %s\n🕒 %s: %s</blockquote>\n"
             "🔑 %s: <code>%s</code>\n\n<i>%s</i>" % (
                 st("VERIFICATION CODE"), sep(),
                 st("From"), esc(sender.get("address") or sender.get("name") or "-"),
                 st("Subject"), esc(msg.get("subject") or "-"),
                 st("Received"), esc(str(msg.get("receivedDateTime") or "-")),
                 st("Code"), esc(code),
                 esc(_clean_preview(msg.get("bodyPreview")))),
             reply_markup=_mail_panel_keyboard(True))
        return
    send(chat_id, "📭 %s" % st("No verification code found in the latest messages."),
         reply_markup=_mail_panel_keyboard(True))


# ── 2FA Code (TOTP) ────────────────────────────────────────────────────────
def parse_totp_key(raw):
    """Accepts a base32 secret or an otpauth:// URI. Returns (secret, digits, period)."""
    ...
    if value.lower().startswith("otpauth://"):
        ...
        digits = int((query.get("digits") or ["6"])[0] or 6)
        period = int((query.get("period") or ["30"])[0] or 30)
    value = _re.sub(r"[\s\-]", "", value).upper()
    if not value or not _re.fullmatch(r"[A-Z2-7=]+", value):
        raise ValueError("invalid key")
    padded = value.rstrip("=")
    padded += "=" * ((8 - len(padded) % 8) % 8)
    try:
        secret = _b64.b32decode(padded, casefold=True)
...
def totp_code(secret, digits=6, period=30, at=None):
    counter = int((at if at is not None else time.time()) // period)
    mac = _hmac.new(secret, _struct.pack(">Q", counter), _hashlib.sha1).digest()
    offset = mac[-1] & 0x0F
    value = _struct.unpack(">I", mac[offset:offset + 4])[0] & 0x7FFFFFFF
    return str(value % (10 ** digits)).zfill(digits)


def start_2fa_flow(chat_id, user_id):
    shop_states[_skey(chat_id, user_id)] = {"step": "user_2fa_key", "data": {}}
    send(chat_id,
         "🔑 <b>%s</b>\n%s\n%s\n\n<i>%s</i>" % (
             st("2FA CODE"), sep(),
             st("Send your own authenticator setup key (base32) or an otpauth:// link."),
             st("The key is used only to generate the code and is never saved in the database.")))


def _2fa_keyboard():
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("🔄 %s" % st("Refresh Code"), callback_data="shopcode:2fa:refresh"))
    return kb


def _send_totp(chat_id, user_id, secret, digits, period):
    code = totp_code(secret, digits, period)
    remaining = period - int(time.time() % period)
    status = st("Valid") if remaining > 5 else st("Expiring now")
    send(chat_id,
         "🔑 <b>%s</b>\n%s\n<blockquote>"
         "🔐 %s: <code>%s</code>\n⏳ %s: <b>%ss</b>\n📌 %s: %s</blockquote>\n<i>%s</i>" % (
             st("2FA CODE"), sep(),
             st("Code"), code,
             st("Valid for"), remaining,
             st("Status"), status,
             st("Key is kept in memory for 5 minutes only, never stored.")),
         reply_markup=_2fa_keyboard())


def handle_2fa_key_input(chat_id, user_id, text, message=None):
    try:
        secret, digits, period = parse_totp_key(text)
    except ValueError:
        send(chat_id,
             "⛔ <b>%s</b>\n%s\n%s" % (st("INVALID 2FA KEY"), sep(),
                                      st("Send a valid base32 setup key or an otpauth:// link.")))
        return
    try:
        ...
        pass
    _totp_cache[user_id] = (secret, digits, period, time.time() + TOTP_CACHE_TTL)
    _send_totp(chat_id, user_id, secret, digits, period)


def refresh_2fa_code(chat_id, user_id):
    entry = _totp_cache.get(user_id)
    if not entry or entry[3] < time.time():
        _totp_cache.pop(user_id, None)
        send(chat_id, "⛔ %s" % st("Session expired. Send your 2FA key again."))
        start_2fa_flow(chat_id, user_id)
        return
    secret, digits, period, _exp = entry
    _send_totp(chat_id, user_id, secret, digits, period)


# ── Get Code callbacks ─────────────────────────────────────────────────────
def handle_getcode_callback(call, data, uid, chat_id):
    action = data.split(":", 2)[1:]
    section = action[0] if action else ""
    op = action[1] if len(action) > 1 else ""
    if section == "mail":
        if op == "login":
            _ack(call)
            if not ms_configured():
                send(chat_id, "⚠️ %s" % st("Microsoft sign-in is not configured yet."))
                return
            send_mail_login_link(chat_id, uid)
            return
        if op == "check":
            _ack(call, st("Checking..."))
            send_latest_mail_code(chat_id, uid)
            return
        if op == "logout":
            ms_delete_account(uid)
            _ack(call, st("Disconnected."))
            send(chat_id, "🚪 %s" % st("Mailbox disconnected. Sign in again any time."),
                 reply_markup=_mail_panel_keyboard(False))
            return
    if section == "2fa" and op == "refresh":
        _ack(call)
        refresh_2fa_code(chat_id, uid)
        return
    _ack(call)
...
def _dispatch_callback(call):
    ...
    if data.startswith("shopcode:"):
        handle_getcode_callback(call, data, uid, chat_id)
        return
    ...
    if data.startswith("shop_do:"):
        ...
        if qty <= 0 or qty > int(p["stock"]):
            ...
            return
        # Direct buy: the staging row is rebuilt from scratch every time.
        cart_clear(uid)
...
class _HealthHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.split("?", 1)[0] == "/oauth/microsoft/callback":
            try:
                import shop as _shop_mod
                status, html = _shop_mod.handle_oauth_callback(self.path)
            except Exception as exc:
                logger.error("Microsoft OAuth callback failed: %s", exc, exc_info=True)
                status, html = 500, "<h3>Sign-in failed. Please try again from the bot.</h3>"
