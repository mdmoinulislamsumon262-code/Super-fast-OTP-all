...
## 🛒 SHOP SYSTEM (new, modular)

Existing bot-er kono part rewrite kora hoyni. Shop puro alada dui file-e:

- `shop.py` — user shop (products, cart, checkout, orders, history, balance,
  offers/coupons, notifications, support) + all `shop_*` tables.
- `shop_admin.py` — isolated Shop Admin Panel (products, stock, orders,
  delivery, coupons & offers, users & balance, reports, settings, logs).

`main.py`-te sudhu wiring:
- Main menu-te `🎁 Refer` er jaygay `🛒 Shop`.
- Referral ekhon `👤 Profile` → inline `🎁 Referral` button.
- text/photo/document handler-e shop hook (private + admin group).
- startup-e `shop.bind(main)`.

### Key behaviour
- Payment = existing main balance (wallet). Checkout atomic:
  `BEGIN IMMEDIATE` → balance guard + stock guard → order create → debit →
  stock deduct. Fail hole full rollback, duplicate confirm blocked (one-time
  checkout token), insufficient balance rejected.
- Order card auto-forward hoy existing admin group-e
  (setting `payment_forward_chat_id`) — Confirm / Cancel / Refund / Deliver buttons.
- Delivery: TEXT, FILE ba AUTO (auto-delivery inventory). Delivery hole user
  notification pay, order status DELIVERED hoy, delivery log thake.
- Cancel/Refund → balance auto refund + stock restore.
- Coupons: PERCENT / FIXED, min amount, total usage, per-user limit, expiry.
- Shop Admin access sudhu ADMIN_ID (+ configured shop admins).
- Sob admin action `shop_activity_logs`-e log hoy.

### Tested (offline simulation)
Main menu (all buttons), Get Number, Temp Mail, Balance, Traffic, Support,
Custom Range, Leaderboard, Profile→Referral, /admin, /tempmail, /checkmail,
product add/list, user product browse, buy-now, checkout, balance deduction,
stock deduction, duplicate-checkout block, insufficient-balance block,
admin-group order card, admin confirm, text delivery, my orders, history,
shop balance, notifications, offers, support, cart, coupon create, and every
shop-admin submenu.
